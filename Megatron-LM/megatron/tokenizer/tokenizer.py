...
def build_tokenizer(args):
    ...
    if args.tokenizer_type == 'BertWordPieceLowerCase':
        ...
        assert args.vocab_file is not None
        tokenizer = _SentencePieceBPETokenizer(model_file=args.vocab_file)
...
class _SentencePieceBPETokenizer(AbstractTokenizer):
    ...
    def __init__(self, model_file):
        ...
        # vocab_size=250002, special_tokens={'bos_token': '<s>', 'eos_token': '</s>', 'unk_token': '<unk>', 'sep_token': '</s>', 'pad_token': '<pad>', 'cls_token': '<s>', 'mask_token': AddedToken("<mask>", rstrip=False, lstrip=True, single_word=False, normalized=True)
        
        self.eod_id = self.tokenizer.PieceToId('</s>')
        self.new_line = self.tokenizer.PieceToId('<0x0A>')
        self.sep_token = self.tokenizer.PieceToId('</s>')
    ...
    @property
    def vocab(self):
        vocab = {self.tokenizer.IdToPiece(i): i for i in range(self.vocab_size)}
        return vocab
    ...
    def detokenize(self, token_ids):
        ...
        # text = "".join(self.convert_ids_to_tokens(token_ids)).replace("▁"," ")
        return text
